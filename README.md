...
| `nostrfy migrate-strfry` | Migrate a strfry relay database into this relay: reads `strfry export` JSONL (or runs `strfry export` itself with `--strfry-db`) and imports the events with their NIP-09/NIP-29 deletion side effects. The relay must be stopped; a re-run is safe; `--dry-run` shows what would be imported. See the [step-by-step migration guide](docs/MIGRATING-FROM-STRFRY.md) |
