...
- **Works behind TLS-terminating proxies** (nginx, Caddy, Cloudflare Tunnel): WebSocket upgrades are honored via `X-Forwarded-Proto`, and with `server.trusted_proxies` the per-IP caps, connect rate limit, `blockip` and logs use the forwarded client address (IPv6 aggregated by `/64`) instead of the proxy's.
...
| `nostrfy migrate-strfry` | Migrate a strfry relay database into this relay: reads `strfry export` JSONL (or runs `strfry export` itself with `--strfry-db`) and imports the events with their NIP-09/NIP-29 deletion side effects. The relay must be stopped; a re-run is safe; `--dry-run` shows what would be imported |

All commands accept `--config <path>` (default `./nostrfy.toml`).

Sending `SIGHUP` to the daemon reloads the configuration at runtime; the reload is **not all-or-nothing**, so every live setting is applied even when the same edit also changes a startup-only one. Live settings include most limits, server auth settings, the NIP-40 toggle, the REST API concurrency ceiling, `rpc.management_token`/`admin_pubkey`, `blossom.restrict_uploads` and `access.restrict_relay`. A few settings are captured at startup and keep their running values — each changed one is warned about: `server.host`, `server.port`, `server.api_host`, `server.ws_paths`, `server.trusted_proxies`, `rpc.max_admin_body_bytes`, `server.metrics_enabled`, `database.path`, `database.purge_interval_secs`, `database.map_size`/`max_map_size`/`max_dbs`/`max_readers`/`search_index`/`meta_index`/`reader_threads`/`disabled_fsync`, the database request timeouts/queue caps, `max_indexed_words`, `relay.private_key` (a reload warns that it is ignored), `relay.enabled_nips`/`disabled_nips`, `relay.livekit_*`, `relay.max_groups`, `blossom.host`/`storage`/`local_path`/`max_upload_bytes`/`min_free_bytes`/`s3_*`, `daemon.max_log_size_bytes`/`max_log_files`/`stats_interval_secs`/`log_file`/`pid_file`, `live_buffer`/`live_batch_size`/`live_batch_interval_ms`, `socket_recv_buffer_kb`, and the HTTP-layer limits `max_connections`/`max_connections_per_ip`/`http_read_timeout_secs`/`max_connections_per_sec_per_ip` (they shape the accept loop built at startup). An invalid reloaded file is rejected as a whole (the old configuration stays in force). The access control kind/IP lists are runtime-managed (NIP-86) and are **not** overwritten by a reload; `restrict_relay` is config-owned and applies.
...
- A single filter may carry at most **512 `ids`, `authors` or `kinds` entries**; the `#...` tag constraint values share a separate **512-value combined budget** per filter. Larger filters are rejected with `CLOSED invalid: too many ids, authors, kinds or tag values in a filter` (the per-candidate match is linear in these arrays, so unbounded arrays would allow quadratic work per REQ).
- `limits.max_connections_per_sec_per_ip` tracks at most 10,000 source IPs: while that map is full of still-active windows, a previously unseen IP is refused (**fail closed**) instead of let through, so a host with a large address pool cannot bypass the rate limit; already-tracked IPs are always enforced.
...
| [62](https://github.com/nostr-protocol/nips/blob/master/62.md) | Request to vanish (stricter than the spec: a vanished pubkey is recorded permanently and every later event from it is rejected, so it cannot resume publishing here) |
...
- **Single database thread for writes**, with puts merged into batches that share one write transaction (one fsync per batch). Replies are sent only after a successful commit, so an `OK` implies durability. The `database.disabled_fsync = true` escape hatch (LMDB `MDB_NOSYNC`) trades that guarantee away: commits skip the fsync and the writer only force-syncs about once per second, so a power loss or OS crash can lose acknowledged writes and can corrupt the database (keep the default unless you run from a backup or a replica you can restore).
