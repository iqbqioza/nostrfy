...
/// applied.
fn alias_int<T: TryFrom<i64> + Default>(v: &toml::Value, key: &str) -> T {
    match v.as_integer().and_then(|i| T::try_from(i).ok()) {
        Some(value) => value,
        None => {
            log::warn!(
                "invalid value {v} for deprecated config key {key}; the field's default is used"
            );
            T::default()
        }
    }
...
impl Config {
    ...
    fn nip_kinds(nip: u16) -> Option<&'static [u64]> {
        match nip {
            ...
            78 => Some(&[78, 30078]),
            84 => Some(&[9802]),
            85 => Some(&[30382, 30383, 30384, 30385, 10040]),
            86 => None,
            87 => Some(&[38000, 38172, 38173]),
            88 => Some(&[1018, 1068]),
            94 => Some(&[1063]),
            98 => Some(&[27235]),
            _ => None,
...
"#;
    ...
    fn advertised_client_nips_track_their_kinds() {
        ...
        for (nip, kinds) in [
            ...
            (78, &[78, 30078][..]),
            (84, &[9802][..]),
            (85, &[30382, 30383, 30384, 30385, 10040][..]),
            (87, &[38000, 38172, 38173][..]),
            (88, &[1018, 1068][..]),
            (94, &[1063][..]),
        ] {
            let access = AccessControl {
