...
impl DbClient {
    ...
    /// Loads a Blossom blob's persisted metadata, distinguishing a database
    /// failure (`None`) from "no mapping" (`Some(None)`): the upload path
    /// must not treat a failed read as "no owner yet" and later roll back a
    /// valid mapping, and a lookup failure must not be reported as 404.
